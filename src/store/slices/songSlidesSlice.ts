...
interface SongSlidesState {
  slides: SongSlide[];
  displaySlides: SongSlide[]; // Pre-computed display sequence, updated only when slides/settings change
...
const songSlidesSlice = createSlice({
  ...
  reducers: {
    setSlides: (state, action: PayloadAction<SongSlide[]>) => {
      ...
      state.displaySlides = createDisplaySequence(
        action.payload,
    ...
    setCurrentDisplayIndex: (state, action: PayloadAction<number>) => {
      ...
      const displaySlide = state.displaySlides[action.payload];
      if (displaySlide) {
    ...
    setRepeatChorusAfterVerse: (state, action: PayloadAction<boolean>) => {
      ...
      state.displaySlides = createDisplaySequence(state.slides, action.payload);
    },
    clearSlides: (state) => {
      state.slides = [];
      state.displaySlides = [];
...
// Selectors
// displaySlides is pre-computed in state, so this selector is O(1)
export const selectDisplaySlides = (state: { songSlides: SongSlidesState }) =>
  state.songSlides.displaySlides;
